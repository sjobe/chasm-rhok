...
class Chasm_Profile_Parser
{
    ...
    public static function generateXYPoints( $data ) {
        ...
        for ($i=0; $i < count($data); $i++) {
            array_push($xyPoints, getNextPoint( $xyPoints[$i][self::X], $xyPoints[$i][self::Y], 
                                                $data[$i][self::H], $data[$i][self::L], 
                                                $data[$i][self::THETA])
           );
        }
    ...
    function generateWaterColumns( $width, $water ) {
        ...
        for ( $x=0; $x < $width; $x++ ) {
            ...
            $x1 = $water[$line_seg_idx-1][self::X];
            $y1 = $water[$line_seg_idx-1][self::Y];
                    
            $x2 = $water[$line_seg_idx][self::X];
            $y2 = $water[$line_seg_idx][self::Y];
    ...
    */
    ...
    public static function getCellValue( $x_coord, $y_coord, $profile, $soil1, $soil2, $soil3, $soil4 ) {
        $soilLayers = array( $soil1, $soil2, $soil3, $soil4 );
        return getCellValueArr( $x_coord, $y_coord, $profile, $soilLayers );
    }
    
    public static function getCellValueArr( $x_coord, $y_coord, $profile, $soilLayers )
    {
        $line_seg_idx = findSegment( $x_coord, $profile );
        
        if ( isAboveLine( $x_coord, $y_coord, 
                $profile[$line_seg_idx-1][self::X], $profile[$line_seg_idx-1][self::Y], 
                $profile[$line_seg_idx][self::X], $profile[$line_seg_idx][self::Y] ) ) {
        ...
        {
            $line_seg_idx = findSegment( $x_coord, $soilLayers[ $i ] );
            if ( isAboveLine( $x_coord, $y_coord, 
