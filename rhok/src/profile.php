...
class Chasm_Profile_Parser
{
	...
	public static function generateWaterColumns( $width, $water ) {
		...
		for ( $x=0; $x < $width; $x++ ) {
			$line_seg_idx = self::findSegment( $x, $water );
	...
	public static function generateCells( $profile, $soil1, $soil2, $soil3, $soil4 ) {
		...
		$x = 0.5;
		while ( $x < $width ) {
			$column = self::generateColumn( $x, $height, $profile, $soil1, $soil2, $soil3, $soil4 );
	 ...
	 */
	// TODO: PHPDoc
	public static function generateFile( $cells, $water_columns ) {

		// TODO: take output stream?
		echo "<pre>";
		echo count($cells) . self::BR;
		...
		for ( $row=0; $row<count($cells); $row++ ) {
			...
			echo $num_cells 
				...
				. " " . $column_width 
				. " " . $column_breadth 
				. " " . $initial_surface_section . self::BR;
		...
		// print EOF marker
		echo "0 " . $water_columns[count($cells) - 1];
		echo "</pre>";
	}
	...
	public static function generateColumn( $x, $height, $profile, $soil1, $soil2, $soil3, $soil4 ) {
		...
		while ($y > 0) {
			array_push($column, self::getCellValue( $x, $y, $profile, $soil1, $soil2, $soil3, $soil4 ));
			$y -= 1.0;
		}
	...
	*/
	...
	public static function getCellValue( $x_coord, $y_coord, $profile, $soil1, $soil2, $soil3, $soil4 ) {
		...
		return self::getCellValueArr( $x_coord, $y_coord, $profile, $soilLayers );
	}
